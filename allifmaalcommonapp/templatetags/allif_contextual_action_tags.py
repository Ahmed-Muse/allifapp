...
def allif_get_contextual_actions_map_previouslhyworkingone(context, allifquery, user_var, glblslug): # Renamed function for clarity
    """
...
@register.simple_tag(takes_context=True)
def allif_get_contextual_actions_map(context, allifquery, user_var, glblslug):
    """
    Prepares and returns a dictionary (map) of contextual action links
    for a specific CommonTransactionsModel instance, based on the company's sector.
    This tag does NOT render HTML; it only provides data. It combines links
    from a 'General' category with those from the specific sector.
    """
    cmpnysctr_raw = context.get('cmpnysctr', 'General')
    
    # Get the sector key, defaulting to 'General' if not found
    if isinstance(cmpnysctr_raw, models.Model) and hasattr(cmpnysctr_raw, '__str__'):
        cmpnysctr_key = str(cmpnysctr_raw)
    else:
        cmpnysctr_key = cmpnysctr_raw

    # Get the general links and the specific sector links
    general_actions = allif_sector_contextual_action_require_pk_links.get('General', [])
    sector_specific_actions = allif_sector_contextual_action_require_pk_links.get(cmpnysctr_key, [])

    # Combine the lists. Specific actions will be appended after general ones.
    all_actions_data = general_actions + sector_specific_actions
    ...
    for action_data in all_actions_data:
        ...
        try:
            ...
            # Use the action name as the key for the final map
            resolved_actions_map[action_name] = {'name': action_name, 'url': current_url}
        
        except NoReverseMatch as e:
            # You might want to log this error in a more robust way in production
            print(f"DEBUG: NoReverseMatch for action '{action_name}' (URL: {action_data['url_name']}): {e}")
            resolved_actions_map[action_name] = {'name': action_name, 'url': '#url_error'}
        except Exception as e:
            print(f"DEBUG: General exception for action '{action_name}': {e}")
            resolved_actions_map[action_name] = {'name': action_name, 'url': '#general_error'}
